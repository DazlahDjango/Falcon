...
async def test_reportplt_consumers():
    user = await User.objects.acreate(
        email="test_reportplt_ws@example.com",
        username="test_reportplt_ws",
    ...
    # 1. DashboardConsumer
    comm_dash = WebsocketCommunicator(application, f"/ws/dashboard/dash-100/?token={token}")
    connected, _ = await comm_dash.connect()
    assert connected
    await comm_dash.disconnect()

    # 2. ReportStatusConsumer
    comm_status = WebsocketCommunicator(application, f"/ws/report/rep-500/status/?token={token}")
    connected, _ = await comm_status.connect()
    assert connected
    await comm_status.disconnect()

    # 3. NotificationConsumer
    comm_notif = WebsocketCommunicator(application, f"/ws/notifications/?token={token}")
