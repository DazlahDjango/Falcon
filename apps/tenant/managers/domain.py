...
class DomainManager(BaseManager):
    ...
    def by_domain(self, domain):
        try:
            return self.get_queryset().get(domain=domain)
        except self.model.DoesNotExist:
            return None

    def by_organization(self, organization_id):
        return self.active().filter(organization_id=organization_id)

    def primary_domains(self):
        return self.active().filter(is_primary=True)

    def get_primary_for_organization(self, organization_id):
        try:
            return self.active().get(organization_id=organization_id, is_primary=True)
        except self.model.DoesNotExist:
            return None
