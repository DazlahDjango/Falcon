import os
import uuid
import logging
from typing import Tuple, Optional
from io import BytesIO
from PIL import Image
from django.core.files.storage import default_storage
from django.core.files.base import ContentFile
from django.conf import settings
from apps.accounts.models import Profile
from apps.accounts.services.audit.logger import audit_service

logger = logging.getLogger(__name__)

class AvatarService:
    def __init__(self):
        self.audit_service = audit_service
        self.max_size = 5 * 1024 * 1024  # 5MB
        self.allowed_formats = ['JPEG', 'PNG', 'GIF', 'WEBP']
        self.avatar_sizes = {
            'thumbnail': (32, 32),
            'small': (64, 64),
            'medium': (128, 128),
            'large': (256, 256),
        }
        self.max_dimension = 500

    def upload_avatar(self, user, file, request=None) -> Tuple[bool, str, Optional[str]]:
        is_valid, error = self._validate_file(file)
        if not is_valid:
            return False, error, None
        try:
            processed_file, filename = self._process_image(file, user.id)
            path = default_storage.save(filename, processed_file)
            url = default_storage.url(path)
            profile = Profile.objects.get(user=user)
            if profile.avatar and profile.avatar.name:
                default_storage.delete(profile.avatar.name)
            profile.avatar = path
            profile.save(update_fields=['avatar'])
            self.audit_service.log(
                user=user, action='profile.avatar_uploaded', action_type='update',
                request=request, severity='info'
            )
            logger.info(f"Avatar uploaded for user {user.email}")
            return True, 'Avatar uploaded successfully', url
        except Profile.DoesNotExist:
            logger.error(f"Profile not found for user {user.email}")
            return False, 'User profile not found', None
        except Exception as e:
            logger.error(f"Avatar upload error for {user.email}: {str(e)}", exc_info=True)
            return False, 'Unable to upload avatar', None

    def update_avatar(self, user, file, request=None) -> Tuple[bool, str, Optional[str]]:
        return self.upload_avatar(user, file, request)

    def delete_avatar(self, user, request=None) -> Tuple[bool, str]:
        try:
            profile = Profile.objects.get(user=user)
            if profile.avatar and profile.avatar.name:
                default_storage.delete(profile.avatar.name)
                profile.avatar = None
                profile.save(update_fields=['avatar'])
                self.audit_service.log(
                    user=user, action='profile.avatar_deleted', action_type='update',
                    request=request, severity='info'
                )
                logger.info(f"Avatar deleted for user {user.email}")
                return True, 'Avatar deleted successfully'
            return False, 'No avatar found'
        except Profile.DoesNotExist:
            logger.error(f"Profile not found for user {user.email}")
            return False, 'User profile not found'
        except Exception as e:
            logger.error(f"Avatar delete error for {user.email}: {str(e)}", exc_info=True)
            return False, 'Unable to delete avatar'

    def get_avatar_url(self, user, size: str = 'medium') -> Optional[str]:
        try:
            profile = Profile.objects.get(user=user)
            if profile.avatar and profile.avatar.name:
                url = profile.avatar.url
                if size in self.avatar_sizes:
                    return f"{url}?size={size}"
                return url
            return None
        except Profile.DoesNotExist:
            return None

    def _validate_file(self, file) -> Tuple[bool, str]:
        if file.size > self.max_size:
            return False, f'File size exceeds {self.max_size // (1024 * 1024)}MB limit'
        try:
            img = Image.open(file)
            if img.format not in self.allowed_formats:
                return False, f'File format not supported. Allowed: {", ".join(self.allowed_formats)}'
            file.seek(0)
            return True, ''
        except Exception:
            return False, 'Invalid image file'

    def _process_image(self, file, user_id) -> Tuple[ContentFile, str]:
        img = Image.open(file)
        if img.mode in ('RGBA', 'LA', 'P'):
            background = Image.new('RGB', img.size, (255, 255, 255))
            if img.mode == 'P':
                img = img.convert('RGBA')
            background.paste(img, mask=img.split()[-1] if img.mode == 'RGBA' else None)
            img = background
        elif img.mode != 'RGB':
            img = img.convert('RGB')
        if img.width > self.max_dimension or img.height > self.max_dimension:
            img.thumbnail((self.max_dimension, self.max_dimension), Image.Resampling.LANCZOS)
        ext = file.name.split('.')[-1].lower()
        if ext not in ['jpg', 'jpeg', 'png', 'gif', 'webp']:
            ext = 'jpg'
        filename = f'avatars/{user_id}/{uuid.uuid4().hex}.{ext}'
        buffer = BytesIO()
        save_format = 'JPEG' if ext in ['jpg', 'jpeg'] else ext.upper()
        img.save(buffer, format=save_format, quality=85, optimize=True)
        buffer.seek(0)
        return ContentFile(buffer.read()), filename

avatar_service = AvatarService()