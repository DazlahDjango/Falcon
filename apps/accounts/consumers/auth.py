...
class AuthConsumer(AsyncWebsocketConsumer):
    ...
    async def disconnect(self, close_code):
        if hasattr(self, 'user_id'):
            ...
            logger.info('Websocket auth disconnected: %s code=%s', self.user_id, close_code)
    ...
    async def receive(self, text_data):
        try:
            ...
            logger.error('Invalid websocket message: %s', text_data)
        except Exception as e:
            logger.error('Websocket receive error: %s', e)

    async def security_event(self, event):
        """Channels handler for AccountsEventBroadcaster."""
        await self.send(text_data=json.dumps({
            'type': 'security_event',
    ...
    async def handle_logout(self):
        ...
        await self.send(text_data=json.dumps({'type': 'logout', 'status': 'success'}))
        await self.close()
