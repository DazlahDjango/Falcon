...
# Base abstract models (simplified for tenant app)
...
class Client(BaseModel):
    ...
    # Domain & branding
    domain = models.CharField(_('custom domain'), max_length=255, blank=True, unique=True, null=True)
    logo = models.ImageField(_('logo'), upload_to='tenant_logos/%Y/%m/', blank=True, null=True)
    favicon = models.ImageField(_('favicon'), upload_to='tenant_favicons/', blank=True, null=True)
    ...
    is_verified = models.BooleanField(_('verified'), default=False, help_text='Email/domain verified')
    ...
    def can_create_users(self):
        ...
        user_count = User.objects.filter(tenant_id=self.id, is_deleted=False).count()
