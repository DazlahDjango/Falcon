...
class HierarchyManager(BaseStructureManager):
    ...
    def get_next_version(self, tenant_id, current_version_id):
        ...
        return self.filter(
            tenant_id=tenant_id,
            version_number__gt=current.version_number,
            is_deleted=False
        ).order_by('version_number').first()
    
    def archive_old_versions(self, tenant_id, keep_count=10):
        versions_to_archive = self.filter(
            tenant_id=tenant_id,
            is_current=False,
            is_deleted=False
    ...
    def get_version_range(self, tenant_id, from_version, to_version):
        return self.filter(
            tenant_id=tenant_id,
            version_number__gte=from_version,
            version_number__lte=to_version,
            is_deleted=False
    ...
    def get_latest_version(self, tenant_id):
        return self.filter(tenant_id=tenant_id, is_deleted=False).order_by('-version_number').first()