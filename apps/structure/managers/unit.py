...
class UnitManager(OrganizationalUnitManager):
    ...
    def with_employments(self, unit_id):
        ...
        return Employment.objects.filter(position__unit_id=unit_id, is_deleted=False, is_active=True, is_current=True)
    ...
    def get_headcount(self, unit_id):
        from apps.structure.models.employment import Employment
        return Employment.objects.filter(position__unit_id=unit_id, is_deleted=False, is_active=True, is_current=True).count()
    
    def get_employments(self, unit_id):
        from apps.structure.models.employment import Employment
        return Employment.objects.filter(position__unit_id=unit_id, is_current=True, is_deleted=False, is_active=True)
    ...
    def by_headcount_range(self, tenant_id, min_count, max_count):
        ...
        unit_ids = Employment.objects.filter(
            ...
            is_deleted=False,
            is_active=True
        ).values('position__unit_id').annotate(count=models.Count('id')).filter(
