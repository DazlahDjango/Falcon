...
class MonthlyActualManager(TenantAwareManager):
    ...
    def adjusted(self):
        return self.filter(status='ADJUSTED')
    ...
    def missing_for_period(self, user_ids, year, month):
        submitted = self.filter(year=year, month=month, user_id__in=user_ids).values_list('user_id', flat=True).distinct()
        return set(user_ids) - set(submitted)

    def get_cumulative(self, user_id, year, month):
        return self.filter(
            user_id=user_id, year=year, month__lte=month, status='APPROVED'
