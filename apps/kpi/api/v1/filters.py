...
class MonthlyActualListFilter(filters.FilterSet):
    ...
    def filter_missing_data(self, queryset, name, value):
        ...
        missing_users = User.objects.filter(
            tenant_id=self.request.tenant.id if hasattr(self, 'request') else None,
            is_active=True
...
class ValidationRecordListFilter(filters.FilterSet):
    ...
    actual_user = filters.UUIDFilter(field_name='actual__user__id')
    actual_kpi = filters.UUIDFilter(field_name='actual__kpi__id')
    actual_year = filters.NumberFilter(field_name='actual__year')
    actual_month = filters.NumberFilter(field_name='actual__month')

    class Meta:
        model = ValidationRecord
        fields = ['status', 'validated_by', 'actual_user', 'actual_kpi', 'actual_year', 'actual_month']
