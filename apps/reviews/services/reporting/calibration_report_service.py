...
class CalibrationReportService(BaseReviewService):
    ...
    def get_cycle_calibration_summary(review_cycle):
        ...
        return {'review_cycle': {'id': str(review_cycle.id), 'name': review_cycle.name}, 'sessions': {'total': sessions.count(), 'completed': sessions.filter(outcome='completed').count(), 'cancelled': sessions.filter(outcome='cancelled').count(), 'list': [{'id': str(s.id), 'name': s.name, 'date': s.scheduled_date.isoformat(), 'status': s.get_status_display(), 'adjustments_count': s.rating_adjustments.count()} for s in sessions.order_by('-scheduled_date')]}, 'calibration_impact': {'total_ratings_calibrated': calibrated_ratings.count(), 'total_ratings_in_cycle': FinalRating.objects.filter(review_cycle=review_cycle).count(), 'percentage_calibrated': round((calibrated_ratings.count() / FinalRating.objects.filter(review_cycle=review_cycle).count()) * 100, 1) if calibrated_ratings.count() > 0 else 0, 'average_before_score': round(avg_before, 2), 'average_after_score': round(avg_after, 2), 'average_change': round(avg_after - avg_before, 2), 'total_adjustments': adjustments.count(), 'increases': adjustments.filter(adjustment_amount__gt=0).count(), 'decreases': adjustments.filter(adjustment_amount__lt=0).count(), 'no_change': adjustments.filter(adjustment_amount=0).count()}}
    @staticmethod
