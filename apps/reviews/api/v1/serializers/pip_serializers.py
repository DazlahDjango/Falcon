...
class PIPSerializer(BaseTenantSerializer):
    ...
    def get_status_display(self, obj):
        status_map = {'draft': 'Draft', 'submitted': 'Active', 'completed': 'Completed', 'approved': 'Approved', 'rejected': 'Rejected', 'cancelled': 'Cancelled', 'archived': 'Archived'}
    ...
    class Meta:
        model = PIP
        fields = [
            'id', 'title', 'description', 'employee', 'employee_name', 'employee_email',
            'owner', 'owner_name', 'review_cycle', 'review_cycle_name', 'severity',
            'severity_display', 'status', 'status_display', 'start_date', 'end_date',
            'extended_to_date', 'extension_reason', 'improvement_areas', 'success_criteria',
            'success_metrics', 'consequences_if_failed', 'consequences_if_successful',
            'employee_acknowledged_at', 'employee_comments', 'manager_signed_at',
            'hr_signed_at', 'outcome', 'outcome_display', 'outcome_notes', 'completed_at',
            'days_remaining', 'completion_percentage', 'is_overdue', 'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'created_at', 'updated_at', 'completed_at', 'employee_acknowledged_at', 'manager_signed_at', 'hr_signed_at']
