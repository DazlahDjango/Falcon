...
from .services.security import ReviewFieldEncryptionService, IntegrityService
from .services.audit import ReviewAuditService

COMMENT_FIELDS = ['comment']
SELF_ASSESSMENT_TEXT_FIELDS = ['overall_comment', 'strengths', 'areas_for_improvement', 'career_aspirations', 'challenges_faced', 'achievements', 'training_completed', 'training_requested']
FEEDBACK_RESPONSE_TEXT_FIELDS = ['strengths', 'areas_for_improvement', 'specific_examples', 'suggestions', 'additional_comments']
...
def encrypt_self_assessment(sender, instance, **kwargs):
    ReviewFieldEncryptionService.encrypt_model_text_fields(instance, SELF_ASSESSMENT_TEXT_FIELDS)
...
def encrypt_feedback_response(sender, instance, **kwargs):
    ReviewFieldEncryptionService.encrypt_model_text_fields(instance, FEEDBACK_RESPONSE_TEXT_FIELDS)
...
def _audit_post_save(sender, instance, created, field_names, **kwargs):
    ...
    ReviewAuditService.log(model_name=sender.__name__, object_id=str(instance.id), action=action, tenant_id=getattr(instance, 'tenant_id', None), instance=instance, checksum_fields=field_names)
...
def audit_delete(sender, instance, **kwargs):
    ReviewAuditService.log(model_name=sender.__name__, object_id=str(instance.id), action=ReviewAuditService.ACTION_DELETE, tenant_id=getattr(instance, 'tenant_id', None))