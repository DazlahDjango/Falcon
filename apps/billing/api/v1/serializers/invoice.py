...
class InvoiceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Invoice
        fields = ['id', 'invoice_number', 'tenant_id', 'subscription_id', 'invoice_date', 'due_date', 'paid_at', 'subtotal', 'subtotal_display', 'tax_rate', 'tax_amount', 'tax_display', 'total_amount', 'total_display', 'currency', 'status', 'status_display', 'is_overdue_status', 'line_items', 'pdf_url', 'pdf_generated_at', 'notes', 'created_at', 'updated_at']
        read_only_fields = ['id', 'invoice_number', 'created_at', 'updated_at', 'pdf_generated_at']
    def get_total_display(self, obj): return obj.total_display
    def get_subtotal_display(self, obj): return f"{obj.currency} {obj.subtotal / 100:.2f}"
    def get_tax_display(self, obj): return f"{obj.currency} {obj.tax_amount / 100:.2f}"
    def get_is_overdue_status(self, obj):
        return {'is_overdue': obj.is_overdue, 'days_overdue': (timezone.now().date() - obj.due_date.date()).days if obj.is_overdue else 0}

class InvoiceListSerializer(InvoiceSerializer):
    class Meta(InvoiceSerializer.Meta):
        fields = ['id', 'invoice_number', 'total_display', 'currency', 'invoice_date', 'due_date', 'status', 'status_display', 'is_overdue_status', 'pdf_url']
...
class InvoiceDetailSerializer(InvoiceSerializer):
    ...
    def get_formatted_line_items(self, obj):
        ...
        for item in obj.line_items:
            formatted.append({'description': item.get('description', ''), 'quantity': item.get('quantity', 1), 'unit_price': item.get('unit_price', 0), 'unit_price_display': f"{obj.currency} {item.get('unit_price', 0) / 100:.2f}", 'total': item.get('total', 0), 'total_display': f"{obj.currency} {item.get('total', 0) / 100:.2f}", 'is_tax': item.get('is_tax', False)})
        return formatted
...
class InvoiceDownloadSerializer(serializers.Serializer):
    format = serializers.ChoiceField(choices=[('pdf', 'PDF'), ('csv', 'CSV'), ('json', 'JSON')], default='pdf')
