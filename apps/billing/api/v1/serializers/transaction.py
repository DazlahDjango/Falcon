...
class TransactionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Transaction
        fields = [
            'id', 'reference', 'paystack_reference', 'tenant_id',
            'subscription_id', 'invoice_id', 'transaction_type', 'type_display',
            'amount', 'amount_display', 'tax_amount', 'tax_display',
            'total_amount', 'total_display', 'currency', 'status', 'status_display',
            'payment_method', 'card_last4', 'card_brand', 'payment_date',
    ...
    def get_total_display(self, obj):
        return f"{obj.currency} {obj.total_amount / 100:.2f}"
...
class TransactionListSerializer(TransactionSerializer):
    ...
    class Meta(TransactionSerializer.Meta):
        fields = [
            ...
            'amount_display', 'total_display', 'status', 'status_display',
            'payment_date', 'created_at'
        ]


class TransactionDetailSerializer(TransactionSerializer):
    """Full detail serializer with additional data."""
    ...
    class Meta(TransactionSerializer.Meta):
        fields = TransactionSerializer.Meta.fields + [
            'is_successful', 'can_refund', 'paystack_response', 'metadata'
        ]
    
    def get_can_refund(self, obj):
        """Check if transaction can be refunded."""
        # Can refund within 90 days of successful transaction
        if obj.status == TransactionStatus.SUCCESS and obj.payment_date:
            from django.utils import timezone
            from datetime import timedelta
            refund_window = timezone.now() - timedelta(days=90)
            return obj.payment_date > refund_window
        return False
