...
class SubscriptionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Subscription
        fields = ['id', 'subscription_code', 'tenant_id', 'plan', 'plan_id', 'status', 'status_display', 'is_active_status', 'start_date', 'trial_end_date', 'current_period_start', 'current_period_end', 'billing_interval', 'amount', 'amount_display', 'currency', 'auto_renew', 'cancel_at_period_end', 'cancelled_at', 'ended_at', 'created_at', 'updated_at']
        read_only_fields = ['id', 'subscription_code', 'tenant_id', 'created_at', 'updated_at', 'cancelled_at', 'ended_at', 'start_date']
    def get_is_active_status(self, obj):
        return {'is_active': obj.is_active, 'is_on_trial': obj.is_on_trial, 'trial_days_remaining': obj.trial_days_remaining, 'days_until_expiry': obj.days_until_expiry, 'is_expiring_soon': obj.is_expiring_soon}
...
class SubscriptionDetailSerializer(SubscriptionSerializer):
    ...
    class Meta(SubscriptionSerializer.Meta):
        fields = SubscriptionSerializer.Meta.fields + ['plan_detail', 'recent_transactions', 'upcoming_invoice', 'paystack_subscription_code', 'paystack_authorization_code']
    ...
    def get_upcoming_invoice(self, obj):
        if obj.is_active and obj.current_period_end > timezone.now():
            return {'amount': obj.amount, 'amount_display': f"{obj.currency} {obj.amount / 100:.2f}", 'due_date': obj.current_period_end, 'days_until_due': obj.days_until_expiry}
        return None
...
class SubscriptionCreateSerializer(serializers.Serializer):
    ...
    trial_days = serializers.IntegerField(default=14, min_value=0, max_value=30)
