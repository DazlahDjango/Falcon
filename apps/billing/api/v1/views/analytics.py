...
class BillingAnalyticsViewSet(viewsets.GenericViewSet):
    ...
    def subscription_analytics(self, request):
        ...
        return Response({'total_active': active, 'total_trialing': trialing, 'total_expired': expired, 'total_cancelled': cancelled, 'by_plan': list(by_plan), 'by_plan_type': {}, 'monthly_recurring_revenue': mrr, 'monthly_recurring_revenue_display': f"KES {mrr/100:.2f}", 'yearly_recurring_revenue': 0, 'yearly_recurring_revenue_display': "KES 0.00", 'total_mrr': mrr, 'total_mrr_display': f"KES {mrr/100:.2f}", 'recent_activity': []})
