...
from apps.billing.default_billing_system_settings import DEFAULT_BILLING_SYSTEM_SETTINGS

CACHE_KEY = 'billing:system_settings:v1'
CACHE_TTL = 300
...
class BillingSettingsService:
    ...
    def get_defaults(cls) -> dict:
        return copy.deepcopy(DEFAULT_BILLING_SYSTEM_SETTINGS)

    @classmethod
    def get_record(cls) -> BillingSystemSettings:
        record, _ = BillingSystemSettings.objects.get_or_create(
            singleton_key=BillingSystemSettings.SINGLETON_KEY,
    ...
    @classmethod
    @transaction.atomic
    def update_settings(cls, patch: dict, user_id=None) -> BillingSystemSettings:
        record = cls.get_record()
    ...
    @classmethod
    def reset_to_defaults(cls, user_id=None) -> BillingSystemSettings:
