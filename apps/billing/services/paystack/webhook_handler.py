...
from ...models import (
    Transaction, Subscription, Invoice, WebhookEventLog,
    PaymentMethod, BillingAuditLog
)
...
class WebhookHandler:
    """
    Handles specific PayStack webhook event types.
    Each event type has its own processing logic.
    """
    ...
    def handle_charge_success(self, event_data: Dict[str, Any], webhook_log: WebhookEventLog) -> Optional[Transaction]:
        ...
        This is the most common webhook - payment completed successfully.
        """
        ...
        if transaction_obj.subscription:
            ...
            # If subscription was past due, reactivate
        ...
        if auth_data.get('authorization_code'):
            PaymentMethod.objects.update_or_create(
                ...
                defaults={
                    'tenant_id': transaction_obj.tenant_id,
                    'customer_code': auth_data.get('customer_code', ''),
                    'email': data.get('customer', {}).get('email', ''),
                    'payment_type': 'card',
                    'card_last4': auth_data.get('last4', ''),
                    'card_brand': auth_data.get('brand', ''),
                    'card_expiry_month': auth_data.get('expiry_month', ''),
                    'card_expiry_year': auth_data.get('expiry_year', ''),
                    'reusable': auth_data.get('reusable', True),
                    'status': 'active',
                }
            )
        ...
        BillingAuditLog.log_action(
            ...
            after=serialize_for_audit(transaction_obj),
            metadata={'webhook_source': 'charge.success'}
        )
    ...
    def handle_subscription_create(self, event_data: Dict[str, Any], webhook_log: WebhookEventLog) -> Optional[Subscription]:
        ...
        New subscription created in PayStack.
        """
        ...
        if subscription:
            ...
            BillingAuditLog.log_action(
                ...
                metadata={'webhook_source': 'subscription.create'}
            )
    ...
    def handle_subscription_disable(self, event_data: Dict[str, Any], webhook_log: WebhookEventLog) -> Optional[Subscription]:
        ...
        if subscription:
            ...
            BillingAuditLog.log_action(
                ...
                resource_id=subscription.id,
                after=serialize_for_audit(subscription),
                metadata={'webhook_source': 'subscription.disable'}
            )
    ...
    def handle_subscription_enable(self, event_data: Dict[str, Any], webhook_log: WebhookEventLog) -> Optional[Subscription]:
        ...
        if subscription:
            ...
            BillingAuditLog.log_action(
                ...
                resource_id=subscription.id,
                after=serialize_for_audit(subscription),
                metadata={'webhook_source': 'subscription.enable'}
            )
    ...
    def handle_invoice_payment_failed(self, event_data: Dict[str, Any], webhook_log: WebhookEventLog) -> Optional[Invoice]:
        ...
        if invoice:
            ...
            # Update related subscription to past_due
            ...
            BillingAuditLog.log_action(
                ...
                after=serialize_for_audit(invoice),
                metadata={'webhook_source': 'invoice.payment_failed'}
            )
    ...
    def dispatch(self, event_type: str, event_data: Dict[str, Any], webhook_log: WebhookEventLog):
        ...
        handlers = {
            ...
            'subscription.enable': self.handle_subscription_enable,
