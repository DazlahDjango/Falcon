...
from ...exceptions import SubscriptionNotFoundError, SubscriptionAlreadyActiveError, SubscriptionExpiredError, SubscriptionCancellationError, SubscriptionError
from ...utils import generate_subscription_code, serialize_for_audit, calculate_total_amount, calculate_tax
...
class SubscriptionLifecycleService:
    ...
    @transaction.atomic
    def create_subscription(self, tenant_id: str, plan: SubscriptionPlan, payment_method_code: Optional[str] = None, trial_days: int = 14) -> Subscription:
        existing = Subscription.objects.get_current_for_tenant(tenant_id)
        if existing and existing.is_active:
            raise SubscriptionAlreadyActiveError(f"Tenant already has active subscription: {existing.subscription_code}")
        now = timezone.now()
        ...
        subscription = Subscription.objects.create(
            tenant_id=tenant_id, plan=plan, subscription_code=generate_subscription_code(),
            status=status, start_date=now, trial_end_date=trial_end, current_period_start=now,
            current_period_end=current_period_end, billing_interval=plan.billing_interval,
            amount=plan.price, currency=plan.currency, auto_renew=True, metadata={'source': 'manual_creation'}
        )
        ...
        self.audit_logger.log(user=None, tenant_id=tenant_id, action='create', resource_type='subscription', resource_id=subscription.id, after=serialize_for_audit(subscription), metadata={'subscription_code': subscription.subscription_code})
        return subscription
    ...
    def activate_subscription(self, subscription: Subscription) -> Subscription:
        ...
        self.audit_logger.log(user=None, tenant_id=subscription.tenant_id, action='update', resource_type='subscription', resource_id=subscription.id, after=serialize_for_audit(subscription), metadata={'action': 'activate'})
        return subscription
    ...
    def cancel_subscription(self, subscription: Subscription, at_period_end: bool = True) -> Subscription:
        ...
        self.audit_logger.log(user=None, tenant_id=subscription.tenant_id, action='cancel', resource_type='subscription', resource_id=subscription.id, after=serialize_for_audit(subscription), metadata={'at_period_end': at_period_end})
        return subscription
    ...
    def expire_subscription(self, subscription: Subscription) -> Subscription:
        ...
        self.audit_logger.log(user=None, tenant_id=subscription.tenant_id, action='update', resource_type='subscription', resource_id=subscription.id, after=serialize_for_audit(subscription), metadata={'action': 'expire'})
        return subscription
    ...
    def renew_subscription(self, subscription: Subscription) -> Subscription:
        ...
        self.audit_logger.log(user=None, tenant_id=subscription.tenant_id, action='renew', resource_type='subscription', resource_id=subscription.id, after=serialize_for_audit(subscription), metadata={'invoice_id': str(invoice.id)})
        return subscription
