...
class TrialService:
    ...
    @transaction.atomic
    def start_trial(self, tenant_id: str, plan: SubscriptionPlan, trial_days: Optional[int] = None) -> Subscription:
        ...
        subscription = lifecycle.create_subscription(tenant_id=tenant_id, plan=plan, trial_days=trial_days)
    ...
    def extend_trial(self, subscription: Subscription, extra_days: int = 7) -> Subscription:
        ...
        audit_logger.log(user=None, tenant_id=subscription.tenant_id, action='update', resource_type='subscription', resource_id=subscription.id, after={'trial_end_date': subscription.trial_end_date.isoformat()}, metadata={'action': 'extend_trial', 'extra_days': extra_days})
    ...
    def convert_trial_to_paid(self, subscription: Subscription) -> Subscription:
        ...
        audit_logger.log(user=None, tenant_id=subscription.tenant_id, action='update', resource_type='subscription', resource_id=subscription.id, after={'status': subscription.status}, metadata={'action': 'convert_trial_to_paid'})
