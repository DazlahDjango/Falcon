...
class WebhookProcessor:
    ...
    def process(self, request) -> Optional[WebhookEventLog]:
        ...
        idempotency_key = generate_idempotency_key(event_data.get('id', ''), event_type)
        ...
        if existing_log and existing_log.is_processed:
            ...
            return existing_log
        webhook_log = WebhookEventLog.objects.create(event_type=event_type, event_idempotency_key=idempotency_key, paystack_event_id=event_data.get('id', ''), paystack_data_id=event_data.get('data', {}).get('id', ''), raw_payload=payload, signature_valid=is_valid, processing_status=WebhookEventLog.PROCESSING_STATUS_PENDING)
        try:
            with transaction.atomic():
                ...
                BillingAuditLog.log_action(user=None, tenant_id=None, action='webhook', resource_type='webhook', resource_id=webhook_log.id, after={'event_type': event_type, 'status': 'processed'}, metadata={'idempotency_key': idempotency_key})
