...
import { DASHBOARD_API, WEBSOCKET_PATHS } from '../../config/constants/dashboardApiConstants';

const API_BASE_URL = import.meta.env.VITE_API_URL || '/api/v1';
const DASHBOARD_API_BASE = `${API_BASE_URL}/dashboard`;
...
const recordFailure = () => {
  ...
  if (failureCount >= CIRCUIT_BREAKER_THRESHOLD) {
    ...
    console.warn(`[DashboardService] Circuit breaker opened`);
  }
};
...
apiClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (status === 403) {
      store.dispatch(showToast({ message: 'You do not have permission to access this dashboard', type: 'error' }));
    } else if (status === 429) {
      store.dispatch(showToast({ message: 'Too many requests. Please try again later.', type: 'error' }));
    } else if (status >= 500) {
      store.dispatch(showToast({ message: 'Dashboard server error. Please try again.', type: 'error' }));
    }
...
export { apiClient, withRetry, BaseDashboardService };