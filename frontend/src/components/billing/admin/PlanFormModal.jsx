...
export const PlanFormModal = ({ isOpen, onClose, plan, onSuccess }) => {
    ...
    useEffect(() => {
        if (plan) setFormData({ ...formData, ...plan, yearly_price: plan.yearly_price || null });
        else setFormData({ name: '', plan_type: 'basic', price: 0, yearly_price: null, currency: 'KES', description: '', max_users: 10, max_kpis: 50, max_departments: 10, max_storage_mb: 100, custom_branding: false, api_access: false, sso_enabled: false, advanced_analytics: false, priority_support: false, display_order: 0, is_active: true });
    }, [plan, isOpen]);
    ...
    const handleSubmit = async (e) => {
        ...
        setLocalError(null);
        try {
            if (plan) await updatePlan(plan.id, formData);
            else await addPlan(formData);
            onSuccess();
            onClose();
            ...
            <div className="plan-modal">
                ...
                <form onSubmit={handleSubmit}>
                    <div className="plan-modal-body">
                        ...
                        <div className="form-row"><div className="form-group"><label>Monthly Price (cents)</label><input type="number" value={formData.price} onChange={(e) => setFormData({ ...formData, price: parseInt(e.target.value) || 0 })} /></div><div className="form-group"><label>Yearly Price (cents)</label><input type="number" value={formData.yearly_price || ''} onChange={(e) => setFormData({ ...formData, yearly_price: e.target.value ? parseInt(e.target.value) : null })} /></div><div className="form-group"><label>Currency</label><select value={formData.currency} onChange={(e) => setFormData({ ...formData, currency: e.target.value })}><option value="KES">KES</option><option value="USD">USD</option><option value="NGN">NGN</option><option value="GHS">GHS</option><option value="ZAR">ZAR</option></select></div></div>
