...
import { useCompetencies, useRatingScales } from '../../../../hooks/reviews';
...
const SupervisorReviewCompetencyRating = ({ 
  ratings = [], 
  onChange, 
  disabled = false,
  selfRatings = [],
  employeeRole = null, // Optional: 'staff' | 'manager' | 'supervisor' | 'champion' | 'dashboard_champion' | 'hr_admin'
}) => {
  const { data: serverCompetencies = [], fetchAll: fetchCompetencies } = useCompetencies();
  const [expanded, setExpanded] = useState(true);
  const [localRatings, setLocalRatings] = useState({});

  // Detect evaluation tier of the employee being reviewed
  const appraisalTier = useMemo(() => {
    if (employeeRole) {
      const er = employeeRole.toLowerCase();
      if (er === 'champion' || er === 'dashboard_champion' || er === 'hr_admin' || er === 'hr') return 'champion';
      if (er === 'staff') return 'staff';
      return 'manager';
    }

    // Auto-detect tier based on selfRatings if present
    if (Array.isArray(selfRatings) && selfRatings.length > 0 && Array.isArray(serverCompetencies)) {
      const selfCompIds = selfRatings.map(r => String(r.competency_id || r.competency));
      
      const matchChamp = serverCompetencies.some(c => 
        selfCompIds.includes(String(c.id)) && 
        (CHAMPION_CATEGORY_NAMES.includes(c.category_name || c.category?.name) || c.description?.toLowerCase().includes('champion'))
      );
      if (matchChamp) return 'champion';

      const matchStaff = serverCompetencies.some(c => 
        selfCompIds.includes(String(c.id)) && 
        (STAFF_CATEGORY_NAMES.includes(c.category_name || c.category?.name) || c.description?.toLowerCase().includes('staff'))
      );
      if (matchStaff) return 'staff';

      const matchMgr = serverCompetencies.some(c => 
        selfCompIds.includes(String(c.id)) && 
        (MANAGER_CATEGORY_NAMES.includes(c.category_name || c.category?.name) || c.description?.toLowerCase().includes('manager'))
      );
      if (matchMgr) return 'manager';
    }

    return 'staff'; // Default for direct reports is staff
  }, [employeeRole, selfRatings, serverCompetencies]);
  ...
  // Build self-ratings map for comparison
  const selfRatingMap = useMemo(() => {
    const map = {};
    if (Array.isArray(selfRatings)) {
      selfRatings.forEach(r => {
        const id = r.competency_id || r.competency;
        if (id) {
          map[id] = {
            score: r.raw_score !== undefined ? r.raw_score : (r.score || null),
            comment: r.comment || ''
          };
        }
      });
  ...
  return (
    <div className="supervisor-review-competency-rating" style={{ background: '#ffffff', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '20px', marginBottom: '24px', boxShadow: '0 2px 4px rgba(0,0,0,0.03)' }}>
      {/* Header Bar */}
      <div 
        className="supervisor-review-rating-header"
      ...
      >
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>
                Performance Competencies & Factors
              </h3>
              <span style={{ 
                fontSize: '11px', 
                fontWeight: 600, 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div style={{ textAlign: 'right' }}>
            <span style={{ fontSize: '12px', fontWeight: 700, color: completionPercentage === 100 ? '#16a34a' : '#6d28d9' }}>
            ...
            <div style={{ width: '100px', height: '5px', background: '#f1f5f9', borderRadius: '999px', overflow: 'hidden', marginTop: '3px' }}>
              <div style={{ width: `${completionPercentage}%`, height: '100%', background: completionPercentage === 100 ? '#10b981' : '#6d28d9', transition: 'width 0.3s ease' }} />
            </div>
          </div>
      ...
      {expanded && (
        <div className="competencies-category-groups" style={{ display: 'flex', flexDirection: 'column', gap: '20px', marginTop: '16px' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', background: '#f8fafc', padding: '12px 16px', borderRadius: '8px', border: '1px solid #e2e8f0', alignItems: 'center' }}>
            <span style={{ fontSize: '12px', fontWeight: 700, color: '#334155', marginRight: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}>
              <Info size={14} color="#2563eb" /> Scale Reference:
            </span>
            {LIKERT_LEVELS.map(lvl => (
              <span key={lvl.value} style={{ fontSize: '11px', color: lvl.color, fontWeight: 600, background: lvl.bg, padding: '2px 8px', borderRadius: '4px', border: `1px solid ${lvl.border}` }}>
                {lvl.label}
              </span>
            ))}
          </div>
              ...
              <div key={categoryName} style={{ border: '1px solid #f1f5f9', borderRadius: '8px', overflow: 'hidden' }}>
                ...
                <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                  {items.map((competency, idx) => {
                    ...
                    const employeeRating = selfRatingMap[idKey] || selfRatingMap[competency.id];
                    ...
                    return (
                      ...
                      >
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                          ...
                          {employeeRating && employeeRating.score && (
                            <div style={{ textAlign: 'right', minWidth: '120px' }}>
                              <span style={{ fontSize: '10px', color: '#64748b', display: 'block' }}>Employee Self-Score:</span>
                              <span style={{ fontSize: '12px', fontWeight: 700, color: '#2563eb' }}>
                                {employeeRating.score} / 5
                              </span>
                        ...
                        {/* Optional Supervisor Feedback Comment */}
                        <input
                          type="text"
                          disabled={disabled}
                          placeholder="Supervisor comments or performance evidence..."
