// frontend/src/components/reports/widgets/WidgetCreate.jsx
import React, { useState, useEffect, useCallback } from 'react';
import { useNavigate, useLocation } from 'react-router-dom';
import { FiArrowLeft, FiSave, FiPlus, FiTrash2 } from 'react-icons/fi';
import { useWidgets, useDashboards } from '../../../hooks/reports';
...
export const WidgetCreate = () => {
    ...
    const handleDataConfigChange = (field, value) => {
        setFormData((prev) => ({
            ...prev,
            data_config: { ...prev.data_config, [field]: value },
        }));
    };

    const handleStyleConfigChange = (field, value) => {
        setFormData((prev) => ({
            ...prev,
            style_config: { ...prev.style_config, [field]: value },
        }));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (result) {
                navigate(`/reports/widgets/${result.id}`);
            }
        } catch (err) {
    ...
    const dataSources = [
        ...
        { value: 'reviews', label: 'Review Data' },
        { value: 'tasks', label: 'Task Data' },
        { value: 'pip', label: 'PIP Data' },
    ...
    return (
        <div className="widget-form-container">
            <div className="widget-form-header">
                ...
                <h1 className="page-title">Create Widget</h1>
                <button
            ...
            <form className="widget-form" onSubmit={handleSubmit}>
                ...
                <div className="form-section">
                    ...
                    <div className="form-row">
                        <div className="form-group full-width">
                            <label htmlFor="data_config">Data Config (JSON)</label>
                            <textarea
                                id="data_config"
                                value={JSON.stringify(formData.data_config, null, 2)}
                                onChange={(e) => {
                                    try {
                                        const parsed = JSON.parse(e.target.value);
                                        handleChange('data_config', parsed);
                                    } catch {
                                        // Invalid JSON, keep as is
                                    }
                                }}
                    ...
                    <div className="form-row">
                        <div className="form-group full-width">
                            <label htmlFor="style_config">Style Config (JSON)</label>
                            <textarea
                                id="style_config"
                                ...
                                onChange={(e) => {
                                    try {
                                        const parsed = JSON.parse(e.target.value);
                                        handleChange('style_config', parsed);
                                    } catch {
                                        // Invalid JSON, keep as is
                                    }
                                }}
