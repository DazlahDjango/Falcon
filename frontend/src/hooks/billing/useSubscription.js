...
export const useSubscription = (options = { autoFetch: true }) => {
    ...
    const fetchCurrent = useCallback(() => dispatch(fetchCurrentSubscription()), [dispatch]);
    ...
    const fetchUsage = useCallback((id) => dispatch(getSubscriptionUsage(id)), [dispatch]);
    ...
    useEffect(() => { if (options.autoFetch) fetchCurrent(); }, [options.autoFetch, fetchCurrent]);
