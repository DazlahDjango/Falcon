...
import {
    ...
    fetchAdminRevenue, fetchAdminSubscriptions, setDateRange, clearAnalytics, clearError,
} from '../../store/billing/slices/analyticsSlice';
import {
    selectBillingSummary, selectRevenueReport, selectSubscriptionAnalytics,
    selectAdminRevenueAnalytics, selectAdminSubscriptionAnalytics, selectAnalyticsLoading,
...
export const useBillingAnalytics = (options = { autoFetch: true }) => {
    ...
    const adminRevenue = useSelector(selectAdminRevenueAnalytics);
    const adminSubscriptions = useSelector(selectAdminSubscriptionAnalytics);
    ...
    const fetchAdminRev = useCallback((year = null) => dispatch(fetchAdminRevenue(year)), [dispatch]);
    const fetchAdminSubs = useCallback(() => dispatch(fetchAdminSubscriptions()), [dispatch]);
    ...
    return {
        summary, revenue, subscriptions, adminRevenue, adminSubscriptions, loading, error,
        mrr, mrrDisplay, totalRevenue, totalRevenueDisplay, activeSubscriptions, churnRate, successRate,
        fetchSummary, fetchRevenue, fetchSubscriptions, fetchSubscriptionAnalytics: fetchSubscriptions,
        fetchAdminRevenue: fetchAdminRev, fetchAdminSubscriptions: fetchAdminSubs,
