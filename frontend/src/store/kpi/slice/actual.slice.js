...
export const createActual = createAsyncThunk(
  ...
  async (payload, { rejectWithValue }) => {
    try {
      ...
      const evidenceFile = payload?.evidenceFile || payload?.evidence || (Array.isArray(payload?.evidenceFiles) ? payload.evidenceFiles[0] : null);
...
export const approveActual = createAsyncThunk(
  ...
  async (payload, { rejectWithValue }) => {
    try {
      ...
      const comment = typeof payload === 'object' ? payload?.comment : '';
...
export const rejectActual = createAsyncThunk(
  ...
  async (payload, { rejectWithValue }) => {
    try {
      const id = typeof payload === 'object' ? payload?.id : payload;
      const reasonId = typeof payload === 'object' ? payload?.reasonId : null;
      const comment = typeof payload === 'object' ? payload?.comment : '';
